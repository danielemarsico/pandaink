...
- [ ] **C1 — End-of-download detection is wrong for Slate/Folio** (`sync.js` `readOfflinePenData()`)
  ...
  - [ ] Accept both end markers in the RX handler: `0xc9` (Spark/Intuos Pro) **and**
        `0xc8` with payload byte 0 == `0xed` (Slate/Folio)
  - [ ] Ignore-but-validate the *first* `0xc8` that arrives right after
  ...
  - [ ] Extract the CRC from the Slate `0xc8 [0xed, …]` payload (bytes after `0xed`,
...
- [ ] **C2 — Stroke-file parser can't parse Slate data** (`sync.js` `parseStrokeData()`)
  ...
  - [ ] Rewrite the classification loop to mirror `StrokeDataType.identify` order:
  ...
  - [ ] Compute packet size uniformly as `1 + popcount(header)` for header-classified
        packets (matches Python `StrokePacket` subclasses)
  - [ ] Fix StrokePoint parsing: use the **original header with low 2 bits cleared**
  ...
  - [ ] Keep the delta-accumulation math as is (it already matches Python)
  - [ ] Skip lost-point packets without emitting a point
